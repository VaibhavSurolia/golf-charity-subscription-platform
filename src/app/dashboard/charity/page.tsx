...
export default async function CharityPage() {
  ...
  const { data: profile } = await supabase
    .from("users")
    .select("charity_id, created_at, subscription_status")
    .eq("id", user?.id)
    .single();
  ...
  // 3. User's Personal Impact for the selected Charity (FINAL)
  // Use adminSupabase to bypass any restrictive RLS
  ...
  // 3b. Current Month Personal Impact
  const personalImpact = recordedPersonalImpact;

  // 4. Global Platform-Wide Impact
  const { data: allPayouts } = await adminSupabase
    .from("charity_payouts")
    .select("amount");

  const finalCommunityTotal = allPayouts?.reduce((acc, p) => acc + Number(p.amount), 0) || 0;

  // 5. Prepare Charities list with totals (Pure Ledger)
  const charities = charitiesData?.map((c: any) => {
    const payoutsTotal = c.charity_payouts.reduce((acc: number, p: any) => acc + Number(p.amount), 0);

    return {
      ...c,
      totalRaised: payoutsTotal
    };
  }) || [];
        ...
        <Card className="p-6 border-white/5 bg-gradient-to-br from-emerald-500/10 to-transparent relative overflow-hidden group">
          ...
          <div className="flex items-start justify-between relative z-10">
            <div>
              ...
              <p className="text-[10px] text-emerald-400/60 font-medium mt-1">Total lifetime impact from your contributions</p>
            </div>
        ...
        <Card className="p-6 border-white/5 bg-gradient-to-br from-blue-500/10 to-transparent relative overflow-hidden group">
          ...
          <div className="flex items-start justify-between relative z-10">
            <div>
              ...
              <h3 className="text-4xl font-black text-white">₹{finalCommunityTotal.toLocaleString(undefined, { minimumFractionDigits: 2 })}</h3>
